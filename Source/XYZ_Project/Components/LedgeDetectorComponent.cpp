...
bool ULedgeDetectorComponent::DetectLedge(OUT FLedgeDescription& LedgeDescription)
{
	...
	float DrawTime = 5.0f;
	if (!XYZTraceUtils::SweepCapsuleSingleByChannel(GetWorld(), ForwardCheckHitResult, ForwardCheckStartLocation, ForwardCheckEndLocation, ForwardCheckCapsuleRadius, ForwardCheckCapsuleHalfHeight, FQuat::Identity, ECC_Climbing, QuerryParams, FCollisionResponseParams::DefaultResponseParam, true, DrawTime))
	{
		return false;
	}
	...
	if (!XYZTraceUtils::SweepSphereSingleByChannel(GetWorld(), DownwardCheckHitResult, DownwardCheckStartLocation, DownwardCheckEndLocation, DownwardCheckSphereRadius, ECC_Climbing, QuerryParams, FCollisionResponseParams::DefaultResponseParam, true, DrawTime))
	{
		return false;
	}
	...
	if (XYZTraceUtils::OverlapCapsuleAnyByProfile(GetWorld(), OverlapLocation, OverlapCapsuleRadius, OverlapCapsuleHalfHeight, FQuat::Identity, FName("Pawn"), QuerryParams, true, DrawTime))
	{
		return false;
	}
